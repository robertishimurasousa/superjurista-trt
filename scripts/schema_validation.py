...
def validate_schema_value(value: Any, schema: dict, path: str = "") -> list[str]:
    ...
    if isinstance(value, dict):
        ...
        for field, child_value in value.items():
            ...
            if child_schema is not None:
                child = f"{path}.{field}" if path else field
                issues.extend(validate_schema_value(child_value, child_schema, child))
    ...
    if isinstance(value, list):
        ...
        if isinstance(item_schema, dict):
            for index, item in enumerate(value):
                issues.extend(validate_schema_value(item, item_schema, f"{path}[{index}]"))
