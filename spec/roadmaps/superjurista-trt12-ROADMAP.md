...
| DOM-03 | Claim and requested-remedy matrix | 4 | `IN_PROGRESS` | At least 95% recall during calibration and 100% claim coverage on the final acceptance sample | [`runtime/domain/labor-claim-taxonomy.json`](../../runtime/domain/labor-claim-taxonomy.json), [`runtime/contracts/schemas/claim-matrix.v1.schema.json`](../../runtime/contracts/schemas/claim-matrix.v1.schema.json), [`scripts/build_claim_matrix.py`](../../scripts/build_claim_matrix.py), [`scripts/extract_pje_claim_matrix.py`](../../scripts/extract_pje_claim_matrix.py), `tests/test_claim_matrix_builder.py`, and `tests/test_pje_claim_matrix_extraction.py`. The protected single-case run links eight claimant positions to seven defenses from one explicitly bound respondent document; it records eight missing requested-remedy gaps, four unsupported claim labels, and one claim without a matching defense. The case data remains outside the repository. This is a partial, unreviewed matrix, not a recall or coverage acceptance result; exact remedy extraction, blind legal review, and approved multi-case calibration remain required |
...
1. Extract the exact requested remedies from the final prayer for relief and link them to the eight source-backed claims; keep ambiguous subitems for human review and update the protected matrix without overwriting its prior version.
