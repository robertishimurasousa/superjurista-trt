...
| ID | Deliverable | Points | Status | Acceptance gate | Evidence |
|---|---|---:|---|---|---|
| DOM-01 | Labor document classification | 3 | `IN_PROGRESS` | Approved fixture set meets the calibrated document-type target and preserves unknown type | [`runtime/domain/labor-document-classification.json`](../../runtime/domain/labor-document-classification.json), [`runtime/contracts/schemas/document-classification.v1.schema.json`](../../runtime/contracts/schemas/document-classification.v1.schema.json), [`runtime/providers/pje-pdf-segments.v1.schema.json`](../../runtime/providers/pje-pdf-segments.v1.schema.json), [`scripts/classify_labor_documents.py`](../../scripts/classify_labor_documents.py), [`scripts/segment_pje_pdf.py`](../../scripts/segment_pje_pdf.py), `tests/test_labor_document_classifier.py`, and `tests/test_pje_pdf_segmentation.py`; the focused synthetic suite covers core and generic TRT12 labels, explicit unknown, precedence, normalization, conflicts, deterministic ordering, source-text exclusion, native PJe-outline segmentation, fail-closed malformed outlines, protected output, and the executable CLI. One authorized TRT12 consolidated PDF produced 49 outline-bound documents: 29 classified (59.2%), 20 explicit unknown, and zero conflicts without tracking process content. The remaining unknown labels are certificates, notices, generic documents, and one unqualified objection; qualified legal review and an approved multi-case calibration target remain required before acceptance |
| DOM-02 | Procedural timeline and labor report | 3 | `IN_PROGRESS` | Dates, parties, procedural phase, claims, defenses, and source locators pass blind sample review | [`runtime/contracts/schemas/procedural-timeline.v1.schema.json`](../../runtime/contracts/schemas/procedural-timeline.v1.schema.json), [`runtime/contracts/schemas/labor-report.v1.schema.json`](../../runtime/contracts/schemas/labor-report.v1.schema.json), [`scripts/build_procedural_timeline.py`](../../scripts/build_procedural_timeline.py), [`scripts/build_labor_report.py`](../../scripts/build_labor_report.py), [`scripts/extract_labor_positions.py`](../../scripts/extract_labor_positions.py), [`scripts/extract_labor_defenses.py`](../../scripts/extract_labor_defenses.py), [`scripts/extract_pje_labor_report.py`](../../scripts/extract_pje_labor_report.py), `tests/test_procedural_timeline_builder.py`, `tests/test_labor_report_builder.py`, `tests/test_labor_position_extraction.py`, `tests/test_labor_defense_extraction.py`, and `tests/test_pje_labor_report_extraction.py`; the deterministic suites cover every supported labor-document type, source locators, unknown and conflict gaps, exact PDF/segment/classification/timeline custody, metadata-to-initial-party reconciliation, multiple respondents and defense documents, portable tribunal and instance inputs, protected output, exact section-heading recognition, wrapped headings, Portuguese ordinal markers, stable grouped position IDs, and executable CLIs. The authorized TRT12 process produced a schema-valid protected report with three source-linked parties, the identified first-instance knowledge phase and court unit, all 49 dated events, eight claimant positions, and seven defense positions. Four claim categories remain explicitly `unmapped_`, and the defense preserves the absence of a specific joint/subsidiary-liability answer instead of fabricating one. Both global position-kind gaps are closed, but the v1 report does not assert per-claim coverage. No process content was tracked. Blind legal review against an approved multi-case TRT12 fixture set remains required before acceptance |
| DOM-03 | Claim and requested-remedy matrix | 4 | `IN_PROGRESS` | At least 95% recall during calibration and 100% claim coverage on the final acceptance sample | [`runtime/domain/labor-claim-taxonomy.json`](../../runtime/domain/labor-claim-taxonomy.json), [`runtime/contracts/schemas/claim-matrix.v1.schema.json`](../../runtime/contracts/schemas/claim-matrix.v1.schema.json), [`scripts/build_claim_matrix.py`](../../scripts/build_claim_matrix.py), [`scripts/extract_labor_remedies.py`](../../scripts/extract_labor_remedies.py), [`scripts/extract_pje_claim_matrix.py`](../../scripts/extract_pje_claim_matrix.py), [`scripts/prepare_claim_blind_review.py`](../../scripts/prepare_claim_blind_review.py), `tests/test_claim_matrix_builder.py`, `tests/test_labor_remedy_extraction.py`, `tests/test_pje_claim_matrix_extraction.py`, and `tests/test_claim_blind_review_packet.py`. A protected single-case version preserves the original matrix and links 16 final-prayer entries, including eight numbered subitems, to all eight claims with page-level evidence. All eight missing-remedy gaps are removed. The matrix still has four unsupported claim labels, one unsupported remedy set, and one claim without a matching defense. The protected evidence retains two conditional alternatives and six unmatched ancillary lettered items for review. A separate protected, source-only blank review form was prepared without reading the system outputs; no reviewer has completed or frozen it. No case text is tracked. Qualified legal review and approved multi-case recall/coverage calibration remain required; no acceptance points are awarded yet |
...
| VAL-04 | Acceptance dossier | 2 | `IN_PROGRESS` | Results, limitations, failure modes, and go/no-go recommendation are approved | [`runtime/validation/historical-dossier-review.v1.schema.json`](../../runtime/validation/historical-dossier-review.v1.schema.json), [`runtime/validation/historical-acceptance-dossier.v1.schema.json`](../../runtime/validation/historical-acceptance-dossier.v1.schema.json), [`scripts/build_historical_acceptance_dossier.py`](../../scripts/build_historical_acceptance_dossier.py), and `tests/test_historical_acceptance_dossier.py`; seven deterministic tests bind development, holdout, and correction evidence to the frozen protocol and exact revisions, preserve separate phase results, require limitations and failure modes, reject source drift and material holdout defects, enforce pending/approved/rejected decision identity, and prohibit promotion of rejection or `no_go`. An approved dossier remains local and human-supervised with external judicial actions disabled. Real results and explicit final human approval remain required |
...
| 2026-09-22 | A protected claim matrix requires explicit defense-document-to-respondent binding and preserves missing remedies and unanswered claims | `DOM-03` has one case-level partial artifact without claiming calibrated recall or final claim coverage |
| 2026-09-22 | Final-prayer remedy extraction preserves page-level evidence, alternative relief, and unmatched ancillary requests without overwriting the prior protected matrix | `DOM-03` reduces one-case missing-remedy gaps from eight to zero while retaining taxonomy and missing-defense review gaps; calibration remains pending |
| 2026-09-23 | Independent claim review starts from the custodied source PDF and a blank protected form, without prefilled system predictions | The review instrument is ready; no human inventory, adjudication, recall measurement, or DOM-03 acceptance is claimed |
...
| 2026-09-21 | A normal PJe capture must never provoke an authentication or provider failure merely for coverage | Functional endpoint gaps remain blocking; naturally unobserved failure groups remain visible limitations for separately authorized session and recovery evidence |
...
| BLK-09 | Falcão network rate-limit window triggered during the bounded public integration probe | JUR-02 live acceptance evidence | Temporary; reconfirmed by a second bounded attempt after `OPS-02` acceptance on 2026-09-21, so no further request should be made until the official window changes |
...
1. Assign a qualified reviewer to read and freeze the protected source-only claim inventory before seeing system output; then compare the inventory with the eight claim-to-remedy links, two conditional alternatives, six unmatched ancillary requests, and unsupported taxonomy codes, recording adjudicated corrections in a new protected version.
2. Run the sanitizer against an authorized local TRT12 first-instance capture and review the map for `PJE-01`.
3. Complete the protected case-specific record and pass the executable controlled-pilot preflight.
4. Bind the session-state classifier to one TRT12 probe from the reviewed endpoint map.
5. Bind task and case discovery to the reviewed TRT12 task and process endpoints.
6. Re-run one bounded `JUR-02` live sentence/acórdão custody check after the official Falcão rate-limit window expires.
7. Bind `PJE-04` to the reviewed TRT12 document endpoints and run an authorized closed download rehearsal.
8. Assemble the authorized 20-case sample and reviewer assignment, freeze all pseudonymous claim inventories, score the 15 development cases for `VAL-02`, and keep the five holdout cases unscored until `VAL-03`.
