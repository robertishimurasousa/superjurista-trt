...
| 2026-09-24 | Vincular a revisão final declarada às fontes e a cada pedido | `final-human-review.v1.schema.json`, `prepare_final_human_review.py` e `validate_final_human_review.py` produzem registro pendente privado e conferem hashes, identidade dos pedidos, declaração do revisor e decisões justificadas. Testes sintéticos recusam fonte alterada, pedido duplicado, campos pendentes e colisão da saída; correções permanecem `requires_followup`. O resultado `reviewed_for_consideration` não autentica o revisor, não aprova mérito e não alimenta o controle final. Sem autos autorizados ou revisão jurídica real; avanço ponderado permanece 30,0% |
| 2026-09-24 | Preservar a pendência do resultado na revisão final | Um teste sintético reproduziu `reviewed_for_consideration` indevido quando a declaração era concordante, mas o pedido ainda estava em `pending_human_review`. O validador agora mantém `pending_human_review` e `abstained` em `requires_followup`, mesmo com declaração `agree`. Isso evita confundir conferência declarada com resolução do pedido; não há revisão jurídica real nem novos pontos de aceite, e o avanço ponderado permanece 30,0% |
| 2026-09-24 | Preparar a conferência humana dos itens finais sem associação | `prepare_unmatched_remedy_review.py` cria roteiro e registro pendente protegidos a partir da evidência v2 do PDF consolidado, somente após encontrar inventário cego preenchido. `validate_unmatched_remedy_review.py` confere hashes, identidade de cada item e decisões justificadas contra as fontes atuais; itens materiais ou incertos exigem seguimento. Ensaios usam PDF sintético. O programa não autentica revisor nem confronta automaticamente cada trecho com o texto visual do PDF, não promove a matriz e não torna este registro obrigatório no fluxo PJe por documentos. Revisão qualificada do processo autorizado permanece pendente; `DOM-03` não recebe ponto e o avanço ponderado permanece 30,0% |
| 2026-09-24 | Preservar a fonte dos itens finais sem associação na petição | `requested-remedy-evidence` v2 passa a registrar texto, documento e página dos itens por letra não associados, com esquema catalogado e gravadores que recusam v1 ou IDs divergentes antes da publicação. O contrato v1 permanece histórico; como ele não guardava texto e página desses itens, a passagem para v2 exige nova extração do PDF autorizado. Ensaio técnico em memória no PDF fornecido encontrou 16 itens associados e seis não associados com fonte presente, sem salvar derivados nem revelar trechos. A revisão cega da associação e a calibração com vários processos ainda faltam; `DOM-03` e o avanço ponderado de 30,0% não mudam |
