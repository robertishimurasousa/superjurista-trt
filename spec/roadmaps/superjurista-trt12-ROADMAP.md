...
| DOM-02 | Procedural timeline and labor report | 3 | `IN_PROGRESS` | Dates, parties, procedural phase, claims, defenses, and source locators pass blind sample review | [`runtime/contracts/schemas/procedural-timeline.v1.schema.json`](../../runtime/contracts/schemas/procedural-timeline.v1.schema.json), [`runtime/contracts/schemas/labor-report.v1.schema.json`](../../runtime/contracts/schemas/labor-report.v1.schema.json), [`scripts/build_procedural_timeline.py`](../../scripts/build_procedural_timeline.py), [`scripts/build_labor_report.py`](../../scripts/build_labor_report.py), [`scripts/extract_labor_positions.py`](../../scripts/extract_labor_positions.py), [`scripts/extract_pje_labor_report.py`](../../scripts/extract_pje_labor_report.py), `tests/test_procedural_timeline_builder.py`, `tests/test_labor_report_builder.py`, `tests/test_labor_position_extraction.py`, and `tests/test_pje_labor_report_extraction.py`; the deterministic suites cover every supported labor-document type, source locators, unknown and conflict gaps, exact PDF/segment/classification/timeline custody, metadata-to-initial-party reconciliation, multiple respondents, portable tribunal and instance inputs, protected output, exact section-heading recognition, wrapped headings, Portuguese ordinal markers, stable claim IDs, and executable CLIs. The authorized TRT12 process produced a schema-valid protected report with three source-linked parties, the identified first-instance knowledge phase and court unit, all 49 dated events, and eight source-linked claimant positions. Four positions use current taxonomy labels and four remain explicitly `unmapped_` rather than being silently remapped. The former `missing_claim_position` gap is closed; `missing_defense_position` is the only report gap. No process content was tracked. Defense-position extraction plus blind legal review against an approved multi-case TRT12 fixture set remain required before acceptance |
...
1. Extract source-linked defense positions from the current authorized sample and map them to the eight claimant positions without guessing absent answers.
