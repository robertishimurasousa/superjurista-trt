...
| VAL-02 | Blind historical review | 4 | `IN_PROGRESS` | Approved sample is completed; defects are traceable by stage and severity | [`runtime/validation`](../../runtime/validation), [`scripts/score_historical_reviews.py`](../../scripts/score_historical_reviews.py), [`spec/validation/historical-blind-review-form.md`](../validation/historical-blind-review-form.md), and `tests/test_historical_review_scoring.py`; 12 deterministic tests prove exact 15/5 partition and frozen claim-inventory enforcement, protocol-digest and frozen-category binding, blind attestations, duplicate rejection, unavailable-data reasons without pass imputation, deterministic ordering, schema-valid CLI output, local schema-reference enforcement, severity/defect consistency, stage-level traceability, and automatic failure for critical/high defects. The authorized 20-case review and named qualified reviewer remain required before acceptance |
...
| 2026-09-21 | Historical blind-review scoring is bound to the frozen protocol and keeps development and holdout results separate | `VAL-02` enters progress with deterministic metrics, unavailable-data exclusions, stage/severity custody, and fail-closed critical/high budgets; no points are earned before the authorized human review is complete |
