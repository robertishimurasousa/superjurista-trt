...
| DOM-02 | Procedural timeline and labor report | 3 | `IN_PROGRESS` | Dates, parties, procedural phase, claims, defenses, and source locators pass blind sample review | [`runtime/contracts/schemas/procedural-timeline.v1.schema.json`](../../runtime/contracts/schemas/procedural-timeline.v1.schema.json), [`runtime/contracts/schemas/labor-report.v1.schema.json`](../../runtime/contracts/schemas/labor-report.v1.schema.json), [`scripts/build_procedural_timeline.py`](../../scripts/build_procedural_timeline.py), [`scripts/build_labor_report.py`](../../scripts/build_labor_report.py), `tests/test_procedural_timeline_builder.py`, and `tests/test_labor_report_builder.py`; the deterministic timeline suite covers every supported labor-document type, source locators, unknown and conflict gaps, exact segment/classification custody, protected output, and the executable CLI. The authorized TRT12 process produced 49 dated events with source page ranges and 20 explicit classification gaps; the protected artifact validated against the catalog and no process content was tracked. The report suite continues to cover deterministic ordering, source custody, explicit unknown phase and missing defense, manifest boundaries, duplicate stable IDs, phase evidence, context-schema boundaries, and input immutability. Party and position extraction plus blind legal review against an approved multi-case TRT12 fixture set remain required before acceptance |
